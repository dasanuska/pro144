...
while (cap.isOpened()):
    ...
    mask_1 = cv2.morphologyEx(mask_1, cv2.MORPHO_OPEN, np.ones ((3,3), np.uint8))

    mask_1 = cv2.morphologyEx(mask_1, cv2.MORPHO_DILATE, np.ones ((3,3), np.uint8))

    #Selecting only the part that does not have mask one and saving in mask 2
    mask_2 = cvw.bitwise_not(mask_1)
